...
class TestNoteViews(TestCase):
    def setUp(self):
        ...
        self.note_one = Note.objects.create(title='blarp', description='wat stick', user=self.user)
        self.note_two = Note.objects.create(title='Gnarf', description='wat dat', user=self.user)
    ...
    def test_note_detail_date_filter(self):
        from .views import notes_detail_view
        request = self.request.get('')
        request.user = self.user
        response = notes_detail_view(request, f'{self.note_one.id}')
